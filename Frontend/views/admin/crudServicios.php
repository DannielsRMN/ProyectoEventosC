...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($accion === 'crear') {
            ...
            if ($servicioModel->crearServicio($datos)) $alerta = ['tipo' => 'success', 'msj' => 'Servicio añadido al catálogo'];
            else $alerta = ['tipo' => 'error', 'msj' => 'Error al registrar el servicio.'];
        } 
        elseif ($accion === 'editar') {
            ...
            if ($servicioModel->actualizarServicio($id, $datos)) $alerta = ['tipo' => 'success', 'msj' => 'Servicio actualizado correctamente'];
            else $alerta = ['tipo' => 'error', 'msj' => 'No se pudieron guardar los cambios'];
        }
        elseif ($accion === 'eliminar') {
            $id = intval($_POST['id_eliminar']);
            if ($servicioModel->eliminarServicio($id)) $alerta = ['tipo' => 'success', 'msj' => 'Servicio eliminado del catálogo'];
            else $alerta = ['tipo' => 'error', 'msj' => 'No se puede eliminar: Está en uso.'];
        }
    } catch (Exception $e) { $alerta = ['tipo' => 'error', 'msj' => 'Error: ' . $e->getMessage()]; }
}
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-brand text-white flex items-center gap-3">
                <span class="p-2 bg-rose-600/20 rounded-lg text-rose-400 border border-rose-500/30 shadow-[0_0_15px_rgba(244,63,94,0.3)]">
                    <span class="material-symbols-rounded" style="font-size: 32px;">design_services</span>
                </span>
                CATÁLOGO DE SERVICIOS
            </h1>
            <p class="text-gray-400 mt-2 font-light text-sm pl-1">Administra la oferta de servicios y sus costos base.</p>
        </div>
        ...
        <div class="card-content">
            ...
            <form method="POST" id="form-servicio" class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-5 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-rose-400 uppercase tracking-widest ml-1">Nombre del Servicio</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-rose-500 transition-colors material-symbols-rounded text-xl">edit_note</span>
                ...
                <div class="md:col-span-4 flex flex-col justify-end space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select name="id_proveedor" id="input-proveedor" required class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white pl-10 pr-4 py-3 rounded-lg focus:border-rose-500 outline-none appearance-none cursor-pointer">
                ...
                <div class="md:col-span-3 flex flex-col justify-end space-y-2">
                    ...
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-rose-500 transition-colors material-symbols-rounded text-xl">attach_money</span>
                ...
                <div class="md:col-span-12 flex justify-end pt-4 mt-2 border-t border-white/5">
                    <button type="submit" id="btn-submit" class="bg-rose-600 hover:bg-rose-500 hover:scale-[1.02] active:scale-95 text-white px-8 py-3 rounded-lg font-bold uppercase transition-all shadow-lg shadow-rose-900/40 flex items-center gap-2">
            ...
            <div class="carousel-container" id="serviciosCarousel">
                <?php foreach ($servicios as $s): ?>
                    <div class="carousel-card group hover:border-rose-500/50 hover:shadow-[0_10px_30px_rgba(244,63,94,0.15)]">
                        ...
                        <div class="carousel-img" style="background: linear-gradient(135deg, hsl(<?php echo $bgHue; ?>, 60%, 15%), hsl(<?php echo $bgHue; ?>, 60%, 5%));">
                            <span class="material-symbols-rounded" style="font-size: 60px; color: rgba(244,63,94,0.1);">design_services</span>
                        ...
                        <div class="carousel-body">
                            ...
                            <div class="grid grid-cols-2 gap-2 mt-auto">
                                ...
                                <form method="POST" onsubmit="return confirm('⚠️ ¿ELIMINAR SERVICIO?\n\n🛠️ <?php echo addslashes($s['nombre_servicio']); ?>\n\nEsta acción es irreversible.');" class="w-full">
                                    <input type="hidden" name="accion" value="eliminar">
                                    <input type="hidden" name="id_eliminar" value="<?php echo $s['id_servicio']; ?>">
                                    <button type="submit" class="w-full py-2 rounded bg-gray-800 hover:bg-red-500 hover:text-white text-gray-300 font-bold text-xs uppercase transition-all flex justify-center items-center gap-1">

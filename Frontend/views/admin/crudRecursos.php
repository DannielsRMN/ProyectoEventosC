...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($accion === 'crear') {
            ...
            if ($recursoModel->crearRecurso($datos)) $alerta = ['tipo' => 'success', 'msj' => 'Recurso añadido al inventario.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'Error al crear recurso.'];
        } 
        elseif ($accion === 'editar') {
            ...
            if ($recursoModel->actualizarRecurso($id, $datos)) $alerta = ['tipo' => 'success', 'msj' => 'Inventario actualizado.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'Error al guardar.'];
        }
        elseif ($accion === 'eliminar') {
            $id = intval($_POST['id_eliminar']);
            if ($recursoModel->eliminarRecurso($id)) $alerta = ['tipo' => 'success', 'msj' => 'Recurso eliminado.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'No se puede eliminar: Está en uso.'];
        }
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-brand text-white flex items-center gap-3">
                <span class="p-2 bg-cyan-600/20 rounded-lg text-cyan-400 border border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.3)]">
        ...
        <div class="card-content">
            ...
            <form method="POST" id="form-recurso" class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-6 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-cyan-400 uppercase tracking-widest ml-1">Nombre del Recurso</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-cyan-500 transition-colors material-symbols-rounded text-xl">package_2</span>
                ...
                <div class="md:col-span-6 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-cyan-400 uppercase tracking-widest ml-1">Descripción</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-cyan-500 transition-colors material-symbols-rounded text-xl">description</span>
                ...
                <div class="md:col-span-6 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-cyan-400 uppercase tracking-widest ml-1">Costo Unitario (S/)</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-cyan-500 transition-colors material-symbols-rounded text-xl">attach_money</span>
                ...
                <div class="md:col-span-6 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-cyan-400 uppercase tracking-widest ml-1">Stock Inicial</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-cyan-500 transition-colors material-symbols-rounded text-xl">inventory</span>
                ...
                <div class="md:col-span-12 flex justify-end pt-4 mt-2 border-t border-white/5">
                    <button type="submit" id="btn-submit" class="bg-cyan-600 hover:bg-cyan-500 hover:scale-[1.02] active:scale-95 text-black px-8 py-3 rounded-lg font-bold uppercase transition-all shadow-lg shadow-cyan-900/40 flex items-center gap-2">
                ...
                <div class="module-card group <?php echo $borderColor; ?>">
                    ...
                    <div class="card-content flex flex-col h-full">
                        <div class="flex justify-between items-start mb-6">
                            <div class="icon-box bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 group-hover:bg-cyan-600 group-hover:text-black group-hover:shadow-[0_0_15px_rgba(6,182,212,0.6)]">
                                <span class="font-bold text-lg"><span class="material-symbols-rounded">package_2</span></span>
                            </div>
                            
                            <?php if ($lowStock): ?>
                                <span class="bg-red-500/10 text-red-500 px-3 py-1 rounded-full text-[10px] font-bold border border-red-500/30 animate-pulse flex items-center gap-1">
                                    <span class="material-symbols-rounded text-xs">warning</span> STOCK BAJO
                                </span>
                            <?php else: ?>
                                <span class="text-gray-600 font-mono text-xs tracking-widest">ID #<?php echo str_pad($r['id_recurso'], 3, '0', STR_PAD_LEFT); ?></span>
                            <?php endif; ?>
                        </div>

                        <div class="mb-4">
                            <h3 class="text-lg text-white font-bold mb-1 truncate tracking-wide group-hover:text-cyan-400 transition-colors">
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-6 mt-auto">
                            <div class="bg-black/40 p-3 rounded-lg border border-white/5 text-center group-hover:border-cyan-500/30 transition-colors">
                                <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Costo Unit.</p>
                                <p class="text-cyan-400 font-mono text-lg leading-none">S/ <?php echo number_format($r['costounidad'], 2); ?></p>
                            </div>
                            <div class="bg-black/40 p-3 rounded-lg border border-white/5 text-center group-hover:border-cyan-500/30 transition-colors">
                                <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Stock</p>
                                <p class="text-white font-mono text-lg leading-none <?php echo $lowStock ? 'text-red-500' : ''; ?>"><?php echo $r['stock']; ?></p>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-2 pt-4 border-t border-white/5">
                            ...
                            <form method="POST" onsubmit="return confirm('⚠️ ¿ELIMINAR RECURSO?\n\n📦 <?php echo addslashes($r['nombre_recurso']); ?>\n\nEsta acción es irreversible.');">

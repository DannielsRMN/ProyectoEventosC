...
<head>
    <meta charset="UTF-8">
    <title>Gestión de Recursos - EventosC</title>
    ...
    <style>
        ...
        .recursos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 30px; margin-bottom: 40px; }
        .recurso-card { background: rgba(20, 20, 20, 0.8); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 15px; padding: 30px; transition: all 0.3s ease; }
        .recurso-card:hover { transform: translateY(-5px); border-color: #39ff14; }
        .recurso-icon { font-size: 3rem; color: #39ff14; margin-bottom: 15px; }
        .recurso-nombre { font-family: 'Orbitron', sans-serif; font-size: 1.6rem; color: #ffffff; margin-bottom: 10px; font-weight: 700; }
        .recurso-descripcion { color: rgba(255, 255, 255, 0.7); font-size: 1rem; margin-bottom: 15px; min-height: 40px; }
        .recurso-info { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin: 20px 0; padding: 15px; background: rgba(255, 255, 255, 0.03); border-radius: 10px; }
        ...
        .recurso-acciones { display: flex; gap: 10px; margin-top: 20px; padding-top: 20px; border-top: 1px solid rgba(255, 255, 255, 0.1); }
    ...
    <div class="admin-wrapper">
        <div class="admin-container">
            ...
            <div class="page-header">
                ...
                <a href="index.php?view=crear_recurso" class="btn-crear">
                ...
                <div class="recursos-grid">
                    <?php foreach ($recursos as $recurso): ?>
                        <div class="recurso-card">
                            ...
                            <h2 class="recurso-nombre"><?php echo htmlspecialchars($recurso['nombre_recurso']); ?></h2>
                            <p class="recurso-descripcion"><?php echo htmlspecialchars($recurso['descripcion'] ?? 'Sin descripción'); ?></p>
                            
                            <div class="recurso-info">
                                <div class="info-item">
                                    <span class="info-label">Costo</span>
                                    <span class="info-value">S/ <?php echo number_format($recurso['costounidad'], 2); ?></span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Stock</span>
                                    <span class="info-value"><?php echo htmlspecialchars($recurso['stock']); ?></span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Usado</span>
                                    <span class="info-value"><?php echo htmlspecialchars($recurso['veces_usado']); ?>x</span>
                                </div>
                            </div>
                            
                            <div class="recurso-acciones">
                                <a href="index.php?view=editar_recurso&id=<?php echo $recurso['id_recurso']; ?>" class="btn-editar">
                                    <span class="material-symbols-rounded">edit</span>
                                    Editar
                                </a>
                                <a href="index.php?view=eliminar_recurso&id=<?php echo $recurso['id_recurso']; ?>" 
                                   class="btn-eliminar"
                                   onclick="return confirm('⚠️ ¿ELIMINAR ESTE RECURSO?\n\n📦 Recurso: <?php echo addslashes($recurso['nombre_recurso']); ?>\n\n⚡ Esta acción no se puede deshacer.');">
            ...
            <?php else: ?>
                <div style="text-align: center; padding: 80px 20px; background: rgba(20, 20, 20, 0.8); border-radius: 15px;">
                    ...
                    <h2 style="font-family: 'Orbitron', sans-serif; color: #ffffff; font-size: 2rem;">No hay recursos registrados</h2>
                </div>
            <?php endif; ?>

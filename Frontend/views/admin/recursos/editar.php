...
$id_recurso = intval($_GET['id'] ?? 0);

if ($id_recurso <= 0) {
    echo "<script>alert('❌ Error: ID de recurso no válido.'); window.location.href = 'index.php?view=listar_recursos';</script>";
    exit();
}

$recursoModel = new Recurso();
$recurso = $recursoModel->obtenerRecursoPorId($id_recurso);

if (!$recurso) {
    echo "<script>alert('❌ Error: Recurso no encontrado.'); window.location.href = 'index.php?view=listar_recursos';</script>";
    exit();
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Editar Recurso - EventosC</title>
    ...
    <style>
        ...
        .form-input:focus, .form-textarea:focus { outline: none; border-color: #00c8ff; background: rgba(0, 200, 255, 0.05); }
    ...
    <div class="form-wrapper">
        <div class="form-container">
            <div class="form-header">
                <h1 class="form-title">✏️ Editar Recurso</h1>
            </div>

            <div class="form-card">
                <div class="id-info">
                    <strong>ID de Recurso:</strong> #<?php echo htmlspecialchars($recurso['id_recurso']); ?>
                </div>

                <form method="POST" action="index.php?view=actualizar_recurso">
                    <input type="hidden" name="id_recurso" value="<?php echo htmlspecialchars($recurso['id_recurso']); ?>">

                    <div class="form-group">
                        <label class="form-label" for="nombre_recurso">Nombre del Recurso <span class="required">*</span></label>
                        <input type="text" id="nombre_recurso" name="nombre_recurso" class="form-input" required maxlength="100" value="<?php echo htmlspecialchars($recurso['nombre_recurso']); ?>">
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="descripcion">Descripción</label>
                        <textarea id="descripcion" name="descripcion" class="form-textarea" maxlength="150"><?php echo htmlspecialchars($recurso['descripcion'] ?? ''); ?></textarea>
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="costounidad">Costo por Unidad <span class="required">*</span></label>
                        <input type="number" id="costounidad" name="costounidad" class="form-input" required min="0" max="99999" step="0.01" value="<?php echo htmlspecialchars($recurso['costounidad']); ?>">
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="stock">Stock Disponible <span class="required">*</span></label>
                        <input type="number" id="stock" name="stock" class="form-input" required min="0" max="99999" value="<?php echo htmlspecialchars($recurso['stock']); ?>">
                    </div>

...
if ($id_usuario <= 0) {
    echo "<script>alert('❌ Error: ID de cliente no válido.'); window.location.href = 'index.php?view=listar_clientes';</script>";
    exit();
}

$clienteModel = new Cliente();
$cliente = $clienteModel->obtenerClientePorId($id_usuario);

if (!$cliente) {
    echo "<script>alert('❌ Error: Cliente no encontrado.'); window.location.href = 'index.php?view=listar_clientes';</script>";
    exit();
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Editar Cliente - EventosC</title>
    ...
    <style>
        ...
        .form-hint { font-size: 0.9rem; color: rgba(255, 255, 255, 0.5); margin-top: 8px; }
...
<body>
    ...
    <div class="form-wrapper">
        <div class="form-container">
            ...
            <div class="form-card">
                <div class="id-info">
                    <strong>ID de Cliente:</strong> #<?php echo htmlspecialchars($cliente['id_usuario']); ?>
                </div>

                <form method="POST" action="index.php?view=actualizar_cliente">
                    <input type="hidden" name="id_usuario" value="<?php echo htmlspecialchars($cliente['id_usuario']); ?>">

                    <div class="form-group">
                        <label class="form-label" for="nombre_completo">Nombre Completo <span class="required">*</span></label>
                        <input type="text" id="nombre_completo" name="nombre_completo" class="form-input" required maxlength="100" value="<?php echo htmlspecialchars($cliente['nombre_completo']); ?>">
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="email">Email <span class="required">*</span></label>
                        <input type="email" id="email" name="email" class="form-input" required maxlength="100" value="<?php echo htmlspecialchars($cliente['email']); ?>">
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="password">Nueva Contraseña (opcional)</label>
                        <input type="password" id="password" name="password" class="form-input" placeholder="Dejar en blanco para no cambiar" minlength="6">
                        <div class="form-hint">Solo completa este campo si deseas cambiar la contraseña</div>
                    </div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($accion === 'crear') {
            ...
            if ($proveedorModel->crearProveedor($datos)) $alerta = ['tipo' => 'success', 'msj' => 'Proveedor registrado.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'Error al registrar.'];
        } 
        elseif ($accion === 'editar') {
            ...
            if ($proveedorModel->actualizarProveedor($id, $datos)) $alerta = ['tipo' => 'success', 'msj' => 'Proveedor actualizado.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'Error al actualizar.'];
        }
        elseif ($accion === 'eliminar') {
            $id = intval($_POST['id_eliminar']);
            if ($proveedorModel->eliminarProveedor($id)) $alerta = ['tipo' => 'success', 'msj' => 'Proveedor eliminado.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'No se puede eliminar: Tiene servicios asociados.'];
        }
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-brand text-white flex items-center gap-3">
                <span class="p-2 bg-amber-600/20 rounded-lg text-amber-400 border border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.3)]">
        ...
        <div class="card-content">
            ...
            <form method="POST" id="form-proveedor" class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-6 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-amber-400 uppercase tracking-widest ml-1">Nombre Empresa</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-amber-500 transition-colors material-symbols-rounded text-xl">domain</span>
                ...
                <div class="md:col-span-6 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-amber-400 uppercase tracking-widest ml-1">Contacto Principal</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-amber-500 transition-colors material-symbols-rounded text-xl">person</span>
                ...
                <div class="md:col-span-8 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-amber-400 uppercase tracking-widest ml-1">Dirección Fiscal</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-amber-500 transition-colors material-symbols-rounded text-xl">map</span>
                ...
                <div class="md:col-span-4 flex flex-col justify-end space-y-2">
                    ...
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-amber-500 transition-colors material-symbols-rounded text-xl">call</span>
                ...
                <div class="md:col-span-12 flex justify-end pt-4 mt-2 border-t border-white/5">
                    <button type="submit" id="btn-submit" class="bg-amber-600 hover:bg-amber-500 hover:scale-[1.02] active:scale-95 text-black px-8 py-3 rounded-lg font-bold uppercase transition-all shadow-lg shadow-amber-900/40 flex items-center gap-2">
                ...
                <div class="module-card group">
                    ...
                    <div class="card-content flex flex-col h-full">
                        <div class="flex justify-between items-start mb-6">
                            <div class="icon-box bg-amber-500/10 border border-amber-500/30 text-amber-400 group-hover:bg-amber-600 group-hover:text-black group-hover:shadow-[0_0_15px_rgba(245,158,11,0.6)]">
                                <span class="font-bold text-lg"><?php echo strtoupper(substr($p['nombre_empresa'], 0, 1)); ?></span>
                            </div>
                            <span class="text-gray-600 font-mono text-xs tracking-widest">ID #<?php echo str_pad($p['id_proveedor'], 3, '0', STR_PAD_LEFT); ?></span>
                        </div>

                        <div class="mb-4">
                            <h3 class="text-lg text-white font-bold mb-1 truncate tracking-wide group-hover:text-amber-400 transition-colors">
                        ...
                        <div class="bg-black/40 rounded-lg p-3 space-y-2 mb-6 border border-white/5 group-hover:border-amber-500/20 transition-colors mt-auto">
                        ...
                        <div class="flex gap-2 pt-4 border-t border-white/5">
                            ...
                            <form method="POST" onsubmit="return confirm('⚠️ ¿ELIMINAR PROVEEDOR?\n\n🏢 <?php echo addslashes($p['nombre_empresa']); ?>\n\nEsta acción es irreversible.');">

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($accion === 'crear') {
            ...
            if ($sedeModel->crearSede($datos)) $alerta = ['tipo' => 'success', 'msj' => 'Sede creada correctamente.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'Error al crear la sede.'];
        } 
        elseif ($accion === 'editar') {
            ...
            if ($sedeModel->actualizarSede($id, $datos)) $alerta = ['tipo' => 'success', 'msj' => 'Sede actualizada.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'Error al guardar cambios.'];
        }
        elseif ($accion === 'eliminar') {
            $id = intval($_POST['id_eliminar']);
            if ($sedeModel->eliminarSede($id)) $alerta = ['tipo' => 'success', 'msj' => 'Sede eliminada.'];
            else $alerta = ['tipo' => 'error', 'msj' => 'No se puede eliminar: Tiene eventos asignados.'];
        }
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-brand text-white flex items-center gap-3">
                <span class="p-2 bg-green-600/20 rounded-lg text-green-400 border border-green-500/30 shadow-[0_0_15px_rgba(57,255,20,0.3)]">
        ...
        <div class="card-content">
            ...
            <form method="POST" id="form-sede" class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-4 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-green-400 uppercase tracking-widest ml-1">Nombre de Sede</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-green-500 transition-colors material-symbols-rounded text-xl">store</span>
                ...
                <div class="md:col-span-5 flex flex-col justify-end space-y-2">
                    ...
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-green-500 transition-colors material-symbols-rounded text-xl">map</span>
                ...
                <div class="md:col-span-3 flex flex-col justify-end space-y-2">
                    <label class="text-[10px] font-bold text-green-400 uppercase tracking-widest ml-1">Aforo Máx.</label>
                    <div class="relative group">
                        <span class="absolute left-3 top-3 text-gray-500 group-focus-within:text-green-500 transition-colors material-symbols-rounded text-xl">groups</span>
                ...
                <div class="md:col-span-12 flex justify-end pt-4 mt-2 border-t border-white/5">
                    <button type="submit" id="btn-submit" class="bg-green-600 hover:bg-green-500 hover:scale-[1.02] active:scale-95 text-black px-8 py-3 rounded-lg font-bold uppercase transition-all shadow-lg shadow-green-900/40 flex items-center gap-2">
                ...
                <?php foreach ($sedes as $s): ?>
                    <div class="carousel-card group hover:border-green-500/50 hover:shadow-[0_10px_30px_rgba(57,255,20,0.15)]">
                        ...
                        <div class="carousel-img" style="background: linear-gradient(135deg, hsl(<?php echo $bgHue; ?>, 60%, 15%), hsl(<?php echo $bgHue; ?>, 60%, 5%));">
                            <span class="material-symbols-rounded" style="font-size: 60px; color: rgba(57,255,20,0.1);">apartment</span>
                        ...
                        <div class="carousel-body">
                            ...
                            <div class="bg-white/5 rounded p-2 mb-4 border border-white/5 flex justify-between">
                                <span class="text-[10px] text-gray-500 uppercase">Eventos Totales</span>
                                <span class="text-xs font-bold text-white"><?php echo $s['total_eventos'] ?? 0; ?></span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-2 mt-auto">
                                ...
                                <form method="POST" onsubmit="return confirm('⚠️ ¿ELIMINAR SEDE?\n\n🏢 <?php echo addslashes($s['nombre']); ?>\n\nSi tiene eventos, no se eliminará.');" class="w-full">

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($accion === 'crear') {
            ...
            $stmt->execute([
                $_POST['id_cliente'], $_POST['id_sede'], trim($_POST['nombre_evento']), 
                $_POST['fecha_evento'], $_POST['hora_inicio'], $_POST['hora_fin'], $_POST['estado_evento']
            ]);
        ...
        elseif ($accion === 'editar') {
            ...
            $conexion->prepare($sql)->execute([
                $_POST['id_sede'], trim($_POST['nombre_evento']), $_POST['fecha_evento'], 
                $_POST['hora_inicio'], $_POST['hora_fin'], $_POST['estado_evento'], $_POST['id_evento']
            ]);
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-brand text-white flex items-center gap-3">
                <span class="p-2 bg-indigo-600/20 rounded-lg text-indigo-400 border border-indigo-500/30 shadow-[0_0_15px_rgba(99,102,241,0.3)]">
        ...
        <div class="flex gap-4">
            <a href="index.php?view=dashboard" class="nav-btn group">
                <span class="material-symbols-rounded group-hover:-translate-x-1 transition-transform">arrow_back</span> VOLVER
            </a>
            <button onclick="abrirModal('crear')" class="bg-indigo-600 hover:bg-indigo-500 hover:scale-105 active:scale-95 text-white px-6 py-2 rounded-lg font-bold uppercase transition-all shadow-lg shadow-indigo-900/40 flex items-center gap-2">
                ...
                <div class="module-card group">
                    ...
                    <div class="card-content flex flex-col h-full">
                        <div class="flex justify-between items-start mb-4">
                            <div class="p-2 rounded-lg bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                            ...
                            <span class="px-3 py-1 rounded-full text-[10px] uppercase font-bold border <?php echo $borderEstado; ?>">
                                <?php echo $ev['estado']; ?>
                            </span>
                        </div>

                        <div class="mb-4">
                            <h3 class="text-lg text-white font-brand mb-1 truncate leading-tight group-hover:text-indigo-400 transition-colors">
                        ...
                        <div class="bg-black/40 rounded-lg p-3 space-y-2 mb-4 border border-white/5">
                            ...
                            <div class="flex items-center gap-2 text-xs text-gray-400">
                                <span class="material-symbols-rounded text-indigo-500" style="font-size: 16px;">location_on</span>
                        ...
                        <div class="flex justify-between items-end mb-6 mt-auto px-1 border-t border-white/5 pt-3">
                            <div>
                                ...
                                <span class="text-white font-mono font-bold">S/ <?php echo number_format($ev['costo_total'], 2); ?></span>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] text-gray-500 uppercase">Estado Pago</p>
                                <span class="text-xs font-bold uppercase <?php echo $colorPago; ?>"><?php echo $stPago; ?></span>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <form method="POST" onsubmit="return confirm('⚠️ ¿ELIMINAR EVENTO?\n\nEsta acción eliminará también la reserva asociada.');">
...
<div id="modal" class="modal-overlay">
    <div class="modal-content">
        ...
        <form method="POST" id="form-evento" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select name="id_cliente" id="input-cliente" required class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white pl-10 pr-4 py-3 rounded-lg focus:border-indigo-500 outline-none appearance-none">
                            <option value="">Seleccionar Cliente...</option>
                            <?php foreach ($clientes as $c): ?><option value="<?php echo $c['id_usuario']; ?>"><?php echo $c['nombre_completo']; ?></option><?php endforeach; ?>
                        </select>
                        <span class="absolute right-3 top-3 text-gray-500 material-symbols-rounded pointer-events-none">expand_more</span>
                    </div>
                </div>
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select name="id_sede" id="input-sede" required class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white pl-10 pr-4 py-3 rounded-lg focus:border-indigo-500 outline-none appearance-none">
                            <?php foreach ($sedes as $s): ?><option value="<?php echo $s['id_sede']; ?>"><?php echo $s['nombre']; ?></option><?php endforeach; ?>
                        </select>
                        <span class="absolute right-3 top-3 text-gray-500 material-symbols-rounded pointer-events-none">expand_more</span>
                    </div>
                </div>
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-indigo-400 uppercase tracking-widest ml-1">Nombre del Evento</label>
                <div class="relative group">
                    <span class="absolute left-3 top-3 text-gray-500 material-symbols-rounded">celebration</span>
                    <input type="text" name="nombre_evento" id="input-nombre" required class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white pl-10 pr-4 py-3 rounded-lg focus:border-indigo-500 outline-none placeholder-gray-700" placeholder="Ej. Boda Civil Familia Pérez">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="space-y-2">
                    ...
                    <input type="date" name="fecha_evento" id="input-fecha" required class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-4 py-3 rounded-lg focus:border-indigo-500 outline-none">
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-bold text-indigo-400 uppercase tracking-widest ml-1">Inicio</label>
                    <input type="time" name="hora_inicio" id="input-inicio" required class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-4 py-3 rounded-lg focus:border-indigo-500 outline-none">
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-bold text-indigo-400 uppercase tracking-widest ml-1">Fin</label>
                    <input type="time" name="hora_fin" id="input-fin" required class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-4 py-3 rounded-lg focus:border-indigo-500 outline-none">
                </div>
            </div>
            ...
            <div class="p-4 bg-indigo-900/10 rounded-xl border border-indigo-500/20">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="number" step="0.01" name="costo_total" id="input-costo" required class="w-full bg-black/50 border border-indigo-500/30 text-white p-2 rounded-lg font-mono focus:border-indigo-500 outline-none">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-gray-400 uppercase">A Cuenta (S/)</label>
                        <input type="number" step="0.01" name="monto_pagado" id="input-pagado" required class="w-full bg-black/50 border border-indigo-500/30 text-white p-2 rounded-lg font-mono focus:border-indigo-500 outline-none">
                    </div>
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-[10px] font-bold text-indigo-400 uppercase tracking-widest ml-1">Estado del Evento</label>
                <select name="estado_evento" id="input-estado" class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-4 py-3 rounded-lg focus:border-indigo-500 outline-none">
            ...
            <div class="pt-4 border-t border-white/5 flex justify-end">
                <button type="submit" id="btn-submit" class="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-lg font-bold uppercase transition-all shadow-lg shadow-indigo-900/40 flex items-center gap-2">
